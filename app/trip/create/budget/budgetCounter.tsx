"use client";

import { useState } from "react";

function BudgetCounter({ flag }: { flag: boolean }) {
  const [total, setTotal] = useState("");
  const [error, setError] = useState(false);
  const [save, setSave] = useState(false);

  return (
    <div
      className={`${
        flag ? "block " : "hidden "
      }w-[90%] h-full flex flex-col items-start justify-center gap-5 relative transition-all ease-in-out duration-300`}
    >
      <h4 className="text-white text-[20px] font-bold">
        2. Set a <span className="text-headingBlue">Budget</span>
      </h4>
      <input
        className="md:ml-10 ml-5 w-[45%] md:w-[40%] h-[50px] bg-[#A09E9E] rounded-lg px-5 text-white text-[15px] md:text-[20px] font-bold outline-none"
        placeholder="Trip Name"
        onChange={(e) => setTotal(e.target.value)}
      />
      <div className="w-full h-fit flex flex-row items-center justify-start flex-wrap gap-3">
        <h4
          className="text-white text-[15px] font-bold p-2 pl-6 pr-6 rounded-[5px] bg-green-500 md:ml-10 ml-5 hover:cursor-pointer transition-all ease-in-out duration-300 border-green-500 border-4 hover:bg-white hover:text-green-500"
          onClick={() => {
            if (total.length === 0) {
              setSave(false);
              setError(true);
            } else {
              localStorage.setItem("tripBudget", total);
              setSave(true);
              setError(false);
            }
          }}
        >
          Save
        </h4>
        {error && (
          <h4 className="text-white text-[15px] font-bold p-2 pl-6 pr-6 rounded-[5px] bg-red-500 md:ml-10 ml-5 border-red-500 border-4">
            Please Set Budget
          </h4>
        )}
      </div>
    </div>
  );
}

export default BudgetCounter;
